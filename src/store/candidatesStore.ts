...
interface IFields {
  ...
  name?: string;
  party?: number | { short_name: string; full_name: string };
}
...
export const useCandidatesStore = defineStore({
  id: "candidatesStore",
  state: (): IState => ({
    dataN: [],
    dataNfiltered: [],
  ...
  actions: {
    ...
    async fetchPaginatedCandidates(params: IPaginatedParams): Promise<void> {
      Loading.show();
      $axios
        .get(
          `candidate/${params.offset}/${params.limit}/${params.order}/${params.sort}/${params.keyword}`
        )
        .then((res) => {
          if (res && res.data) {
            this.dataN = res.data.candidates;
    ...
    async editById(): Promise<void> {
      if (this.data && this.data._id) {
        ...
        this.isLoading = true;
        $axios
          .patch(`candidate/${this.data._id}`, diff)
          .then((res) => {
            Loading.hide();
            if (res && res.data) {
              this.isLoading = false;
    ...
    async deleteById(): Promise<void> {
      ...
      if (this.selected.length) {
        ...
        await $axios
          .delete(`candidate/${id_for_delete}`)
          .then(() => {
            Loading.hide();
            Notify.create({
